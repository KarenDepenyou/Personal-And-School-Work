...
public class PublicTests {
	...
	public void longestConsecutiveIdenticalCharacters() {
		assertEquals(1,
				CheckPasswords.longestConsecutiveIdenticalCharacters("ABC2928fhgZ5T"));
		assertEquals(4,
				CheckPasswords.longestConsecutiveIdenticalCharacters("aabaaaahd"));
	...
	public void similarToWordAllowingSubstitutions() {
		assertEquals(true, CheckPasswords.similarToWord("apple", "app1e"));
	...
	/**
	 * Utility method, simplifies calling checkPassword with empty list of banned
	 * words
	 */
	boolean checkPassword(String password) {
		return CheckPasswords.checkPassword(password, new String[0]);
	}
	...
	@Test
	public void mustHave5OrMoreCharsTDictionaryWord() {
		assertEquals(false, CheckPasswords.checkPassword("access14A",
				new String[] { "apple", "access", "butthead" }));
		assertEquals(false, CheckPasswords.checkPassword("Ybutthead0",
				new String[] { "apple", "access", "butthead" }));
		assertEquals(true, CheckPasswords.checkPassword("Bbutthead1234",
				new String[] { "apple", "access", "butthead" }));
		acceptGoodPassword();
	}	
	...
	public void generatePasswords() {
		...
		String password = PasswordGenerator.generatePassword(4, r,
				new String[] {"A", "B", "C", "D"});
		...
		String password3 = PasswordGenerator.generatePassword(3, r,
				new String[] {"A", "B", "C", "D"});
	...
	public void generatePasswordsFromLongerWords() {
		...
		String password = PasswordGenerator.generatePassword(2, r,
				new String[] {"alone", "baked"});
		assertEquals(10, password.length());
		assertTrue(password.equals("alonebaked") || password.equals("bakedalone"));
	}

	/**
	 * Check that the passwords generated aren't all identical. I know we can't
	 * guarantee that the same password won't come up twice by chance, but the
	 * number of possible passwords are such that I'm not too concerned.
	 */
	@Test
