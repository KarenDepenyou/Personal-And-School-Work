...
public class PolymorphicBST<K extends Comparable<K>, V> {
	...
	public Set<K> keySet() {
		Set<K> ans = new HashSet<K>();
//		ans.add(e);
		this.root.addKeysToCollection(ans);
		return ans;
...
//		throw new UnsupportedOperationException("You must implement this method.");
	...
	public PolymorphicBST<K, V> subMap(K fromKey, K toKey) {
		PolymorphicBST<K, V> p = new PolymorphicBST<K, V>();
		p.root = this.root.subTree(fromKey, toKey);
		return p;
	}
	...
	public void rightRootLeftTraversal(TraversalTask<K, V> p) {
		this.root.rightRootLeftTraversal(p);
	}
}